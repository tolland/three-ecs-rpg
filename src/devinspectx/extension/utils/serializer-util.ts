...
import { SceneNodeInfo } from '../../types/message-types';
...
export class Serializer {
   ...
   */
  static serialize(obj: any, options: SerializerOptions = DEFAULT_OPTIONS, depth = 0): any {
    const { maxDepth, includePrivate, includeFunctions, includeSymbols } = { ...DEFAULT_OPTIONS, ...options };
    ...
    if (obj.type && typeof obj.uuid === 'string' && typeof obj.isObject3D !== 'undefined') {
    ...
    if (obj instanceof Set) {
      return {
        __type: 'Set',
        value: Array.from(obj).map(item => this.serialize(item, options, depth + 1))
      };
    }
    ...
    for (const key of keys) {
      ...
      try {
        ...
        if (typeof value === 'function') {
          if (includeFunctions) {
            result[stringKey] = `[Function: ${value.name || 'anonymous'}]`;
          }
          continue;
        }
        ...
        result[stringKey] = `[Error: ${e instanceof Error ? e.message : 'Unknown error'}]`;
      }
    }
   ...
   */
  static toJSON(obj: any, options: SerializerOptions = DEFAULT_OPTIONS): string {
    try {
