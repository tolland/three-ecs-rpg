...
import { PhysicsConfigManager } from '@core/PhysicsConfigManager'; // Could potentially read wind from config
...
export class WindSystem extends System {
    ...
    update(deltaTime: number): void {
        ...
        const currentWind = this.windVector.clone().normalize().multiplyScalar(currentWindStrength);
        // --- End Gust Simulation ---
        // If not simulating gusts, just use: const currentWind = this.windVector;


        const entities = this.world.queryEntities([VelocityComponent, WindAffectedComponent]);

        for (const entity of entities) {
            const vel = this.world.getComponent(entity, VelocityComponent)!;
            const windComp = this.world.getComponent(entity, WindAffectedComponent)!;
            ...
            const windForce = currentWind.clone().multiplyScalar(1 / resistanceFactor);
