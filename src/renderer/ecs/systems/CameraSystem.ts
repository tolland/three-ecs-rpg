...
import {Entity} from "@ecs/Entity"; // Import Octree if passing it directly
...
export class CameraSystem extends System {
    ...
    constructor(world: World, private scene: THREE.Scene) {
        super(world);
    ...
    addCamera(id: string, camera: THREE.PerspectiveCamera, viewport: THREE.Vector4 = new THREE.Vector4(0, 0, 1, 1)): void {
    ...
    private updateCameraProjection(camera: THREE.PerspectiveCamera): void {
        const aspect = (this.rendererSize.x * camera.userData.viewport.z) / (this.rendererSize.y * camera.userData.viewport.w); // Adjust aspect based on viewport dimensions
    ...
    setCameraMode(cameraId: string, mode: CameraMode, targetEntityId?: Entity) {
        ...
        if (!managedCam) {
            console.warn(`CameraSystem: Cannot set mode for unknown camera ID: ${cameraId}`);
            return;
        }

        let entityToModify = targetEntityId ?? managedCam.targetEntity;
        if (entityToModify === null) {
            console.warn(`CameraSystem: Cannot set mode for camera ${cameraId} without a target entity.`);
            return;
        }

        const targetComp = this.world.getComponent(entityToModify, CameraTargetComponent);
        if (targetComp) {
            console.log(`Setting camera mode for entity ${entityToModify} on camera ${cameraId} to ${mode}`);
            ...
            console.warn(`CameraSystem: Target entity ${entityToModify} does not have CameraTargetComponent.`);
        }
    }

    update(deltaTime: number): void {
        const targetEntities = this.world.queryEntities([PositionComponent, CameraTargetComponent]);
        ...
        for (const entity of targetEntities) {
            const targetComp = this.world.getComponent(entity, CameraTargetComponent)!;
        ...
        this.cameras.forEach(managedCam => {
            if (managedCam.targetEntity !== null) {
                const targetPosComp = this.world.getComponent(managedCam.targetEntity, PositionComponent);
                const targetRotComp = this.world.getComponent(managedCam.targetEntity, RotationComponent);
                const targetCamComp = this.world.getComponent(managedCam.targetEntity, CameraTargetComponent);
                ...
                if (targetPosComp && targetRotComp && targetCamComp) {
                    ...
                    this.targetLookAt.copy(this.cameraTargetPosition).add(targetCamComp.lookAtOffset); // Point camera looks at
                    ...
                    switch (targetCamComp.mode) {
                        case CameraMode.FIRST_PERSON:
                            ...
                            const worldOffset = targetCamComp.firstPersonOffset.clone().applyQuaternion(targetRotComp.value);
                            camera.position.copy(this.cameraTargetPosition).add(worldOffset);
                        ...
                        case CameraMode.THIRD_PERSON_ENTITY:
                            ...
                            targetCamComp.currentDistance = THREE.MathUtils.lerp(targetCamComp.currentDistance, targetCamComp.desiredDistance, lerpFactor);
                            targetCamComp.currentDistance = Math.max(targetCamComp.minDistance, targetCamComp.currentDistance); // Clamp min distance
                            ...
                            if (targetCamComp.mode === CameraMode.THIRD_PERSON_ENTITY) {
                                // Rotate offset by entity's rotation *before* adding to position
                                this.idealCameraOffset.applyQuaternion(targetRotComp.value);
                            }

                            this.idealCameraPosition.copy(this.cameraTargetPosition).add(this.idealCameraOffset);


                            // --- Camera Collision ---
                            let finalCameraPosition = this.idealCameraPosition;
                            if (this.worldOctree) {
                                const rayDirection = new THREE.Vector3().subVectors(this.idealCameraPosition, this.targetLookAt).normalize();
                                // Set up the raycaster originating from the lookAt point towards the ideal camera position
                                this.raycaster.set(this.targetLookAt, rayDirection);
                                this.raycaster.far = targetCamComp.currentDistance + targetCamComp.collisionBuffer; // Check up to desired dist + buffer
                                this.raycaster.near = 0.1;

                                // CORRECT: Call the intersection method ON the Octree instance
                                const intersects = this.worldOctree.rayIntersect(this.raycaster.ray);

                                if (intersects) { // Octree.rayIntersect returns a single intersection object or null
                                    ...
                                    const collisionDistance = closestHit.distance - targetCamComp.collisionBuffer;
                                    finalCameraPosition = this.targetLookAt.clone().addScaledVector(rayDirection, Math.max(targetCamComp.minDistance, Math.min(targetCamComp.currentDistance, collisionDistance)));
                                    // Update current distance based on collision, but allow lerping
                                    targetCamComp.currentDistance = Math.min(targetCamComp.currentDistance, collisionDistance);
                                } else {
                                    // No collision, use ideal position
                                    finalCameraPosition = this.idealCameraPosition;
                            ...
                            camera.position.lerp(finalCameraPosition, lerpFactor); // Smooth position changes
    ...
    setTwoPlayerSplitScreen(camId1: string = 'main', camId2: string = 'npc1'): void {
