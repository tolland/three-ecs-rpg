...
import { createTemporaryVisual } from '@renderer/utils/main'; // Import Capsule if using
...
export class CollisionSystem extends System {
  ...
  update(deltaTime: number): void {
    ...
    for (const entity of entities) {
      ...
      this.tempCapsule.start.copy(pos.value).add(new THREE.Vector3(0, collider.radius, 0))
      ...
      if (result) {
        DEBUG_OBJ.scene.add(
          createTemporaryVisual(this.tempCapsule.start,
            DEBUG_OBJ.scene,
            5,
            'yellow'
          ));
        DEBUG_OBJ.scene.add(
          createTemporaryVisual(this.tempCapsule.end,
            DEBUG_OBJ.scene,
            5,
            'pink'
          ));
        ...
        }); // null entity for world collision
