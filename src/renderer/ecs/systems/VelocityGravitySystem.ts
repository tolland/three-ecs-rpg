...
import { GravityAffectedComponent, VelocityComponent, ColliderComponent } from '@ecs/components';
...
export class VelocityGravitySystem extends System {
    ...
    update(deltaTime: number): void {
        const entities = this.world.queryEntities([GravityAffectedComponent, VelocityComponent]);
        ...
        for (const entity of entities) {
            ...
            if (!collider || !collider.onGround) {
                velComp.value.y -= baseGravity * gravComp.gravityMultiplier * deltaTime;
            }
        }
    }
}
